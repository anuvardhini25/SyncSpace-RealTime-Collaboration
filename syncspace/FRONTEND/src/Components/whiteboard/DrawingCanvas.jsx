...
export default function DrawingCanvas({ roomId }) {
  ...
  // Tracks other users' strokes that are still in progress
  ...
  const onStrokeStart = useCallback(
    (stroke) => {
      ...
      connectSocket().emit("whiteboard:startDrawing", {
        roomId,
        stroke,
      });
    },
  ...
  const onStrokePoint = useCallback(
    (point) => {
      ...
      connectSocket().emit("whiteboard:drawing", {
        roomId,
        point,
      });
    },
  ...
  const onStrokeEnd = useCallback(
    (stroke) => {
      ...
      connectSocket().emit("whiteboard:endDrawing", {
        roomId,
        stroke,
      });
    },
  ...
  const clearCanvas = () => {
    ...
    if (roomId) {
      connectSocket().emit("whiteboard:clear", {
        roomId,
      });
    }
  };
    ...
    <div className="w-full h-full flex flex-col relative">
      ...
      <div className="flex-1 w-full min-h-0 overflow-hidden">
        <canvas
          ...
          width={1000}
          height={500}
          className="block w-full h-auto border border-gray-400 bg-white"
        />
      </div>
    </div>
  );
