...
async function performSave(io, roomState) {
  try {
    ...
    // One coalesced replay event per autosave tick (already debounced by
    // SAVE_DEBOUNCE_MS above) rather than one per keystroke - this is
    // the "debounce rapid typing" requirement for the Replay feature.
    logReplayEvent({
      roomId: roomState.roomId,
      type: "CODE_CHANGE",
      action: "UPDATE",
      data: { code: doc.code, language: doc.language },
      userId: roomState.lastEditorId,
      userName: roomState.lastEditorName || "Guest",
    });
  } catch (error) {
    console.error(
...
export default function editorSocket(io, socket) {
  ...
  socket.on("yjsSync", async ({ roomId, update } = {}) => {
    ...
    if (bytes[0] === syncProtocol.messageYjsUpdate) {
      roomState.lastEditorId = socket.user?.id || socket.id;
      roomState.lastEditorName = socket.user?.name || "Guest";
